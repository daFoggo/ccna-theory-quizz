...
import { useQuizMutations } from "@/features/quiz";
import { getTopic, type Question } from "@/lib/questions";
import { cn } from "@/lib/utils";

interface IQuizSessionProps {
	topic: string;
}

export function QuizSession({ topic }: IQuizSessionProps) {
	const navigate = useNavigate();
	const { saveAttempt } = useQuizMutations();
		...
		(key: string) => {
			...
			if (q.type === "multiple-answer") {
				...
				if (next.length >= q.correctAnswers.length && !selected.includes(key)) {
					setSubmitted(true);
					const correct =
						next.length === q.correctAnswers.length &&
						next.every((s) => q.correctAnswers.includes(s));
					const existing = answersRef.current.findIndex(
						(a) => a.question_id === q.qs,
					);
					if (existing >= 0)
	...
	const handlePrevious = useCallback(() => {
		...
		const prevAnswer = answersRef.current.find(
			(a) => a.question_id === prevQ?.qs,
		);
		setIndex(prevIndex);
		setSelected(prevAnswer?.selected_answers ?? []);
		setSubmitted(!!prevAnswer);
			...
			<div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
				...
				<BorderList>
					{questions.map((q) => {
						...
						const sel = a?.selected_answers ?? [];
						return (
							<BorderListItem
								key={q.qs}
								pad="tight"
								tone={correct ? "default" : "muted"}
							>
								<div className="flex items-start gap-3">
									...
									<div className="flex min-w-0 flex-1 flex-col gap-1">
										...
										<p className="text-xs text-muted-foreground">
											...
											{sel
												.map(
													(k) => q.options.find((o) => o.key === k)?.text ?? k,
												)
												.join(", ") || "Nothing"}
										</p>
										{!correct && (
											<p className="text-xs text-chart-1">
												...
												{q.correctAnswers
													.map(
														(k) =>
															q.options.find((o) => o.key === k)?.text ?? k,
													)
													.join(", ")}
											</p>
										)}
				...
				<div className="flex flex-wrap items-center justify-center gap-3">
					...
					{wrongCount > 0 && (
						<Button size="sm" onClick={handleRetryWrong}>
							<IconRotate /> Retry Wrong ({wrongCount})
						</Button>
					)}
				</div>
			</div>
	...
	return (
		<div className="mx-auto flex w-full max-w-lg flex-col gap-6">
			...
			{q.type === "multiple-answer" && !submitted && (
				<p className="text-xs text-muted-foreground">
					Select all that apply, tap selected again to confirm
				</p>
			)}
			...
			<div className="flex flex-col gap-2">
				{q.options.map((opt) => {
					...
					let style =
						"border-border bg-background text-muted-foreground hover:border-primary/40 hover:bg-secondary/50";
					if (submitted && isSelected)
						style = isCorrectOpt
							? "border-chart-1 bg-chart-1/10 text-chart-1"
							: "border-chart-7 bg-chart-7/10 text-chart-7";
					else if (submitted && isCorrectOpt)
						style = "border-chart-1/40 bg-chart-1/5 text-chart-1";
					else if (submitted)
						style = "border-border bg-secondary/30 text-muted-foreground";
					else if (isSelected)
				...
				<div
					className={cn(
						...
						isCorrect
							...
							: "border-chart-7/30 bg-chart-7/5",
					)}
				>
					<p
						className={cn(
							"text-sm font-medium",
							isCorrect ? "text-chart-1" : "text-chart-7",
						)}
					>
				...
				{submitted && (
					<Button onClick={handleNext} disabled={saving}>
						{saving ? (
							...
							<>
								{isLast ? "Finish" : "Next"} <IconChevronRight />
							</>
						)}
					</Button>
				)}
