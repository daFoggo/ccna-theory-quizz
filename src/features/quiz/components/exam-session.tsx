	...
	const handleNext = useCallback(async () => {
		if (isLast) {
			...
			try {
				...
				await saveAttempt.mutateAsync({ topic: "mixed-exam", score, total: questions.length, answers: answersRef.current });
			} catch {
		...
		return (
			<div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
							...
							<BorderListItem key={q.qs} pad="tight" tone={correct ? "default" : "muted"}>
								<div className="flex items-start gap-3">
									...
									<div className="flex min-w-0 flex-1 flex-col gap-1">
										...
										<p className="text-xs text-muted-foreground">You answered: {sel.map((k) => q.options.find((o) => o.key === k)?.text ?? k).join(", ") || "Nothing"}</p>
				...
				<div className="flex items-center justify-center gap-3">
					...
					<Button variant="outline" size="sm" onClick={() => window.location.reload()}><IconRotate /> New Exam</Button>
				</div>
			</div>
		);
	}
		...
		<div className="mx-auto flex w-full max-w-lg flex-col gap-6">
			...
			{submitted && (
				<div className={cn("rounded-lg border p-4", isCorrect ? "border-chart-1/30 bg-chart-1/5" : "border-chart-7/30 bg-chart-7/5")}>
					<p className={cn("text-sm font-medium", isCorrect ? "text-chart-1" : "text-chart-7")}>{isCorrect ? "Correct!" : "Incorrect"}</p>
					{q.explanation && <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{q.explanation}</p>}
				</div>
			)}

			<div className="flex justify-between">
				<Button variant="outline" size="sm" disabled={index === 0} onClick={handlePrevious}><IconChevronLeft /> Previous</Button>
				{submitted && (
