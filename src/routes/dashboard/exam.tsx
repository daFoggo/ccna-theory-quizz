	...
	const handleSelect = useCallback((key: string) => {
		...
		if (q.type === "multiple-answer") {
			...
			if (next.length >= q.correctAnswers.length && !selected.includes(key)) {
				...
				answersRef.current.push({ question_id: q.qs, selected_answers: next, is_correct: correct });
			}
		} else {
			const answer = [key];
			setSelected(answer);
			...
			answersRef.current.push({ question_id: q.qs, selected_answers: answer, is_correct: correct });
		}
