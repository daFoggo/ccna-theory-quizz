...
import { getAttemptResultsFn } from "@/features/quiz";
import { BorderGrid, BorderGridCell, BorderList, BorderListItem, BorderSectionHeader } from "@/components/common/grid";
				...
				<BorderGridCell pad="compact">
					<div className="flex flex-col gap-1">
						...
						<span className={cn(
							...
							pct >= 80 ? "text-chart-1" : pct >= 60 ? "text-chart-3" : "text-chart-7",
						)}>
							{pct}%
						</span>
					</div>
				...
				<BorderList>
					{answers.map((a) => {
						const correctText = a.question?.correctAnswers
							.map((k) => a.question?.options.find((o) => o.key === k)?.text ?? k)
							.join(", ");

						return (
							<BorderListItem key={a.question_id} pad="tight" tone={a.is_correct ? "default" : "muted"}>
								<div className="flex items-start gap-3">
									<div className={cn(
										"mt-0.5 flex size-5 shrink-0 items-center justify-center rounded",
										a.is_correct ? "bg-chart-1/10 text-chart-1" : "bg-chart-7/10 text-chart-7",
									)}>
										{a.is_correct ? <IconCheck className="size-3.5" /> : <IconX className="size-3.5" />}
									</div>
									<div className="flex min-w-0 flex-1 flex-col gap-1">
										...
										<p className="text-xs text-muted-foreground">
											You answered:{' '}
											{a.selected_answers.map((k: string) => a.question?.options.find((o) => o.key === k)?.text ?? k).join(", ") || "Nothing"}
										</p>
										{!a.is_correct && correctText && (
											<p className="text-xs text-chart-1">Correct: {correctText}</p>
										)}
