import { IconArrowRight, IconArrowsShuffle, IconBrain, IconChecks, IconHistory, IconScoreboard } from "@tabler/icons-react";
...
function DashboardHome() {
	...
	const topicAttempts = new Map<string, { scores: number[]; totals: number[]; recent: { score: number; total: number } | null }>();
	for (const a of all) {
		if (a.topic === "mixed-exam") continue;
		const cur = topicAttempts.get(a.topic) ?? { scores: [], totals: [], recent: null };
	...
	const cards = [
		{ label: "Quizzes", value: String(totalAttempts), icon: IconBrain, sub: "Total taken" },
		{ label: "Avg. Score", value: `${avgPct}%`, icon: IconScoreboard, sub: "All time" },
		{ label: "Questions", value: String(totalAnswered), icon: IconChecks, sub: `${totalCorrect} correct` },
		{ label: "Topics", value: String(TOPIC_GROUPS.length), icon: IconHistory, sub: "CCNA domains" },
	];
		...
		<div className="flex flex-col gap-8">
					...
					<BorderGridCell key={card.label} pad="compact">
						<div className="flex flex-col gap-2">
							...
							<div className="flex items-end justify-between">
								<span className="font-heading text-2xl font-semibold tracking-tight">{card.value}</span>
			...
			<BorderGrid cols={2}>
				{TOPIC_GROUPS.map((topic) => {
					...
					const best = data ? Math.max(...data.scores.map((s, i) => Math.round((s / data.totals[i]) * 100))) : null;
					const recent = data?.recent ? Math.round((data.recent.score / data.recent.total) * 100) : null;
								...
								{best !== null && (
									<div className="flex shrink-0 flex-col gap-1 text-xs">
										<span className="flex items-center gap-1.5 rounded-md bg-secondary/50 px-2 py-0.5 text-muted-foreground">
											Best
											<span className={cn("font-semibold tabular-nums", best >= 80 ? "text-chart-1" : "text-chart-3")}>{best}%</span>
										</span>
										{recent !== null && (
											<span className="flex items-center gap-1.5 rounded-md bg-secondary/50 px-2 py-0.5 text-muted-foreground">
												Last
												<span className={cn("font-semibold tabular-nums", recent >= 80 ? "text-chart-1" : recent >= 60 ? "text-chart-3" : "text-chart-7")}>{recent}%</span>
											</span>
										)}
									</div>
								)}
