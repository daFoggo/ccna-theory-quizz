...
export async function refreshAuthToken(options?: { clearOnFailure?: boolean }) {
	...
	if (isServer) {
		return refreshAuthTokenInternal(clearOnFailure);
	}
	...
	clientRefreshPromise = refreshAuthTokenInternal(clearOnFailure).finally(
		() => {
...
async function refreshAuthTokenInternal(clearOnFailure: boolean) {
	try {
...
let tokenCache: { value: string | null; updatedAt: number } | null = null;
